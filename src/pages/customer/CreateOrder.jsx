...
const calcFees = (pickup, dropoff) => {
  if (!pickup || !dropoff) return { deliveryFee: 0, commission: 0, riderNet: 0, distKm: 0 }
  const distKm = Math.sqrt(
    Math.pow((pickup.lat - dropoff.lat) * 111, 2) +
    Math.pow((pickup.lng - dropoff.lng) * 111, 2)
  )
  const deliveryFee = Math.round(distKm * 800 + 1500)
  const commission  = Math.round(deliveryFee * COMMISSION)
  const riderNet    = deliveryFee - commission
  return { deliveryFee, commission, riderNet, distKm: distKm.toFixed(1) }
}
...
async function sendTelegram(order, fees) {
  ...
  const total = Number(order.itemValue) + fees.deliveryFee
  const text = `
...
export default function CreateOrder() {
  ...
  const fees  = calcFees(pickup, dropoff)
  const total = Number(itemValue || 0) + fees.deliveryFee
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        riderNet:      fees.riderNet,
        distKm:        fees.distKm,
        total:         Number(itemValue) + fees.deliveryFee,
        paymentType, note,
        status:        "pending",
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center flex-col gap-4 px-6">
        ...
        <p className="text-xs text-gray-300 bg-gray-100 px-3 py-1 rounded-full">#{orderId?.slice(-6).toUpperCase()}</p>
        <div className="flex gap-3 mt-2 w-full max-w-xs">
          <button onClick={resetForm} className="flex-1 py-4 bg-primary-500 text-white font-bold rounded-2xl shadow-primary">
            ➕ ထပ်တင်မည်
          </button>
          <button onClick={() => navigate("/customer")} className="flex-1 py-4 border border-gray-200 text-gray-600 font-bold rounded-2xl">
            🏠 Home
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-col h-screen bg-surface">
      ...
      <div className="flex-1 overflow-y-auto pb-28">
        ...
        <div className="px-4 mb-4">
          ...
          {pickup && dropoff && (
            <div className="mt-2 bg-primary-50 rounded-2xl px-4 py-2 flex justify-between items-center">
              <span className="text-xs text-gray-500">📏 {fees.distKm} km</span>
              <span className="text-sm font-display font-black text-primary-500">
                Delivery: {fees.deliveryFee.toLocaleString()} ကျပ်
              </span>
            </div>
          )}
        </div>
        ...
        {step === 2 && (
          <div className="px-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {ITEM_TYPES.map(t => (
                  <button key={t.value} onClick={() => setItemType(t.value)}
                    className={`card text-sm py-3 text-left active:scale-[0.98] transition-all
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 mb-1 block uppercase tracking-wider">📞 ဆက်သွယ်ရန် ဖုန်း</label>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setPaymentType("cod")}
                  className={`card py-3 text-sm font-semibold transition-all
            ...
            <div className="card mb-4 space-y-3">
              ...
              <div className="border-t border-gray-100 pt-3">
                <div className="bg-gray-50 rounded-2xl p-3 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-500">🚚 Delivery Fee <span className="text-gray-300">({fees.distKm} km)</span></span>
                    <span className="font-semibold">{fees.deliveryFee.toLocaleString()} ကျပ်</span>
                  </div>
                  <div className="border-t border-gray-200 pt-2 flex justify-between items-center">
                    <span className="text-sm font-bold text-gray-700">💰 Total</span>
                    <span className="text-xl font-display font-black text-primary-500">
                      {total.toLocaleString()} ကျပ်
                    </span>
                  </div>
                </div>
              </div>
